#!/usr/bin/env python3
#python -m PyInstaller --onefile --windowed --name="ScriptLauncher" launcher
...
class GitPushUI:
    ...
    def check_changes(self):
        ...
        if not repo_path or not os.path.exists(repo_path): messagebox.showerror("Error", "Repository path is invalid or does not exist."); return
        self.update_changes_display("Scanning for changes...")
        self.log_message("Checking for changes...")
        stdout, stderr, code = self.git_manager._run_git_command('add --all', repo_path, self.log_message)
        if code != 0: self.update_changes_display(f"An error occurred while staging files.\nSee the Log tab for details."); return
        stdout, stderr, code = self.git_manager._run_git_command('status --porcelain', repo_path, self.log_message)
        if code != 0: self.update_changes_display(f"An error occurred.\nSee the Log tab for details.")
        elif not stdout or stdout.strip() == '': self.update_changes_display("No changes detected.")
        else:
            added_files, modified_files, deleted_files = [], [], []
            for line in stdout.strip().split('\n'):
                if not line: continue
                status, filename = line[:2], line[3:]
                if 'D' in status: deleted_files.append(filename)
                elif 'A' in status or '?' in status: added_files.append(filename)
                elif 'M' in status: modified_files.append(filename)
            changes_text = "=" * 60 + "\nCHANGES SUMMARY\n" + "=" * 60 + "\n\n"
            if added_files: changes_text += f"[+] FILES TO BE ADDED ({len(added_files)}):\n" + "\n".join([f"    + {f}" for f in added_files]) + "\n\n"
            if modified_files: changes_text += f"[~] FILES TO BE MODIFIED ({len(modified_files)}):\n" + "\n".join([f"    ~ {f}" for f in modified_files]) + "\n\n"
            if deleted_files: changes_text += f"[-] FILES TO BE DELETED ({len(deleted_files)}):\n" + "\n".join([f"    - {f}" for f in deleted_files]) + "\n\n"
...
class NanoBananaUI:
    ...
    def create_config_tab(self, parent):
        ...
        prompt_entry = ttk.Entry(config_frame, textvariable=self.prompt_var, width=60, font=("Arial", 10))
        prompt_entry.grid(row=2, column=1, columnspan=2, sticky=tk.EW, pady=8, padx=(5, 0))
    ...
    def load_config_to_ui(self):
        ...
        self.prompt_var.set(config.get('default_prompt', 'make the background pure red'))
        self.log_message("Configuration loaded.")
    def save_config(self):
        config_data = {'api_key': self.api_key_var.get(), 'folder_path': self.folder_path_var.get(), 'default_prompt': self.prompt_var.get()}
    ...
    def start_processing_thread(self):
        ...
        if not all([self.api_key_var.get(), self.folder_path_var.get(), self.prompt_var.get()]):
            messagebox.showwarning("Missing Information", "Please complete all configuration fields."); return
        ...
        thread = threading.Thread(target=self.nano_manager.process_images, args=(self.api_key_var.get(), self.prompt_var.get(), self.folder_path_var.get(), self.log_message), daemon=True)
        thread.start(); self.parent.after(100, self.check_thread, thread)
    def stop_process(self):
